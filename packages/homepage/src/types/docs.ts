...
// Generated at: 2026-07-05T13:12:54.604Z
// Master locale: en
...
export interface DocSectionIds {
  ...
  'custom-fetch-service-worker': 'custom-fetch-intercepting-serviceworker' | 'when-you-need-it' | 'how-it-works' | 'registering-the-serviceworker' | 'the-serviceworker-script' | 'how-it-differs-from-workercachestorage' | 'network-behavior' | 'when-to-use'
  ...
  'reactive-reads': 'reactive-reads-controlled-re-renders' | 'which-tool-when' | 'reading-without-a-re-render-is-not-a-hook'
  ...
  'shared-worker-middleware': 'sharedworkermiddleware' | 'which-one-to-import' | 'what-gets-synchronized' | 'n-stores-over-one-sharedworker' | 'fallback-and-ssr' | 'when-to-use-which' | 'types'
