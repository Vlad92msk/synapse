...
Два бага в `MemoryStorage`/`LocalStorage`, проявлявшиеся на вложенных путях и при записи из
middleware. Обратно совместимо: API и сигнатуры не менялись, поведение стало корректнее.

- **Вложенный `set`/`remove` больше не мутирует переданный `initialState`.** Раньше
  `set('a.b', x)` писал по пути «по месту» и портил общий объект `initialState` (`MemoryStorage`
  держит состояние по ссылке) — после этого `reset()` восстанавливал уже испорченный снапшот.
  `setValueByPath` стал чистым (клонирует только узлы на пути, structural sharing), добавлен
  иммутабельный `removeValueByPath`; адаптеры (`doSet`/`doUpdate`/`doRemove`) переведены на них.
  Теперь `reset()` корректно восстанавливает вложенные поля, а ссылка пользователя на
  `initialState` остаётся нетронутой.
- **`update()` обновляет `getStateSync()` из стора.** Раньше кэш пересобирался из до-`dispatch`
  снимка + изменённых апдейтером ключей, поэтому ключи, дописанные middleware во время dispatch
  (например производный `errors`), терялись в `getStateSync()` / `useStorageSubscribe`. Теперь
  `_stateCache` читается из стора (паритет с `set()`) — производное состояние из middleware видно
  сразу.
- **Тесты** (`core/storage/__tests__/immutable-writes.test.ts`): иммутабельность `setValueByPath`/
  `removeValueByPath`, `reset()` после вложенного `set`/`remove`, неизменность `initialState`,
  и видимость middleware-сайд-записи в `getStateSync()` после `update()` (Memory + LocalStorage).

## Документация — 2026-06-28

### Рецепт «Формы» (управление формами на хранилище synapse)

Копипастный рецепт управления формами без зависимостей — альтернатива `react-hook-form`/`Formik`.

- **Док-страница `forms`** (RU + EN, новая подгруппа «Рецепты» в pillar State Manager) — лесенка
  уровней: базовая форма на `MemoryStorage` → валидация как `SyncMiddleware` → персист черновика
  (`LocalStorage` + `syncBroadcastMiddleware` + `version`/`migrate`) → SSR через `hydrate()`.
  Submit-flow, изоляция ре-рендеров через `useStorageSubscribe({ equals })`, честное сравнение с
  form-библиотеками и альтернатива-селектор для валидации.
- **Тесты рецепта** (`core/storage/__tests__/forms-recipe*.{test.ts,test.tsx}`) фиксируют грабли
  валидационной middleware: не блокировать инвалидный ввод, отсутствие рекурсии при записи `errors`,
  уведомление подписчиков `errors`/`subscribeToAll`, кросс-полевую валидацию, `reset()`, `isValid` и
  изоляцию ре-рендеров по полю в React. Поле пишется вложенным `set('values.<name>', v)` (после
  фикса 5.2.1 это иммутабельно и reset-safe).

## [5.2.0] - 2026-06-28

### Реактивное чтение хранилища: управляемые ререндеры

Закрывает сценарий «меняю стор обычными методами (`set/update`), а в компоненте читаю реактивно, но сам
контролирую ререндеры». Раньше все «читающие» хуки (`useStorageSubscribe`/`useSelector`/`useObservable`)
ререндерили компонент на каждое изменение — не было примитива «держать свежее значение без ререндера».
Изменения аддитивны и обратно совместимы.

- **`useStorageRef(storage, selector?, options?)`** (экспорт из `synapse-storage/react`) — реактивное чтение
  **без** автоматического ререндера. Держит свежий срез в `ref` (обновляется по подписке) и отдаёт контроль
  компоненту: `{ ref, get(), rerender() }`. Три сценария: «ререндер, когда я сам решу» (зови `rerender()`),
  «вообще без ререндера» (`get()` в обработчике события) и «ререндер по условию» (`options.shouldRerender(prev, next)`).
  Не использует `useSyncExternalStore` (он не умеет пропускать ререндер по решению компонента) — осознанно
  без tearing-гарантий Concurrent Mode. RxJS не требует. `handle` (`ref/get/rerender`) стабилен между рендерами.
- **`useStorageSubscribe(storage, selector, options?)`** — добавлена опция `equals?: (a, b) => boolean`.
  Мемоизирует снапшот: если выбранный срез «равен» предыдущему — компонент не ререндерится (паритет с
  `useSelector`). Нужно для селекторов, возвращающих объект/массив (новая ссылка на каждый тик), и для
  изоляции ререндеров по конкретному срезу. Сигнатура без `options` не изменилась.
- **`useStorageObservable(storage, selector?)`** — мемоизирующая обёртка над `toObservable` + `useObservable`
  (RxJS-путь). Снимает footgun: инлайновый `toObservable(storage)` в рендере создавал новый Observable и
  лишние пере-подписки на каждый рендер; здесь поток мемоизируется по `[storage]`. Со `selector` поток
  эмитит только изменения среза.
- **`toObservable(storage, selector, equals?)`** — перегрузка: поток только по выбранному срезу с
  `map + distinctUntilChanged` (по умолчанию `Object.is`, либо переданный `equals`). Без `selector`
  поведение прежнее (весь стейт через `shareReplay(1)`).
