...
export interface BaseStorageConfig<T extends Record<string, any> = Record<string, any>> {
  ...
  migrate?: MigrateFn<T>
  /**
   * Стратегия `hydrate` для SSR-засева/повторной гидрации:
   * - `'replace'` (по умолчанию) — снапшот применяется как единая гранула: полная замена
   *   объекта модуля. Подходит модулю-«одному объекту»; свежесть (`__hydratedAt`) — на весь снапшот.
   * - `'merge'` — shallow-мердж по ВЕРХНЕУРОВНЕВЫМ ключам: ключи снапшота накладываются на
   *   существующее состояние, ключи вне снапшота НЕ трогаются (keyed-кэш `Record<key, …>`, где
   *   ключи из разных источников). Свежесть считается ПО КЛЮЧУ (устаревший ключ не блокирует
   *   свежие), нотификация — только по реально изменившимся ключам.
   */
