...
describe('эффект, эмитящий функцию → функция вызывается', () => {
  it('emit функции исполняется EffectsModule', async () => {
    ...
    class FnEffects extends Effects<State, TestDispatcher> {
      readonly e = this.effect((action$, _s, { dispatcher: d }) => action$.pipe(ofType(d.ping), map(() => sideEffect)))
    }
...
describe('обработка ошибок', () => {
  it('ошибка в одном эффекте не убивает остальные', async () => {
    ...
    class MixedEffects extends Effects<State, TestDispatcher> {
      readonly boom = this.effect(() => new Observable(() => { throw new Error('boom') }))
      readonly ok = this.effect((action$, _s, { dispatcher: d }) => action$.pipe(ofType(d.ping), tap(() => survivor())))
    }
...
describe('совместимость и реестр', () => {
  ...
  it('смешанный массив [инстанс.getEffects(), legacyFn] — оба работают', async () => {
    ...
    const legacy: Effect<State, TestDispatcher> = (action$, _s, { dispatcher }) => action$.pipe(ofType((dispatcher as TestDispatcher).ping), tap(() => legacyHit()))
