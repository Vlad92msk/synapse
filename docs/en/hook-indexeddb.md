...
Same hook with `type: 'indexedDB'`. Returns `IAsyncStorage`. Note: `destroyOnUnmount` defaults to `false` for IndexedDB.
...
function NotesPage() {
  ...
  // useStorageSubscribe works identically for sync and async
  const notes = useStorageSubscribe(storage, (s) => s.notes)

  // set/update — return Promises (but you don't need to await in handlers)
