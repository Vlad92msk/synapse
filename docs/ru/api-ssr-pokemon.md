...
**TL;DR:** на сервере `api.request(...)` прогревает кэш → `api.dehydrate()` в HTML → на клиенте
`api.hydrate(state)` **до** `api.init()`. `useApiQuery` читает засеянную страницу синхронно (без вспышки
loading), пагинация дальше идёт обычными клиентскими запросами. Нужен sync-стор и один инстанс api на
серверный запрос.
...
- **Без вспышки loading нужен sync-стор на клиенте.** Мгновенный первый рендер `useApiQuery` использует
  [`getCachedSync()`](./api-client.md#синхронное-чтение-кэша-endpointgetcachedsync), а он работает только
