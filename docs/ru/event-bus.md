...
- Несколько **независимых** модулей должны реагировать на доменные события, но не должны знать друг
  о друге (аналитика/тосты/бейджи слушают `POKEMON_*`, не импортируя pokemon-синапс).
- Нужны **wildcard-подписки** (`POKEMON_*`, `*`), приоритеты, TTL, история событий.
- Хочется **влить** внешние события в поток действий другого synapse — через `externalDispatchers`
  (см. в конце).

## Когда НЕ нужно

- Общение **внутри одного модуля** — это диспетчер/эффекты, шина тут лишний слой.
- Между двумя модулями нужен **прямой типизированный** вызов — проще cross-store зависимость
  (см. [dependencies](./dependencies.md)), без косвенности по строковым именам событий.
- Кросс-**вкладочная** синхронизация состояния — это не про шину, а про
  `syncBroadcastMiddleware`/`syncSharedWorkerMiddleware` (см. [middlewares](./middlewares.md)).
