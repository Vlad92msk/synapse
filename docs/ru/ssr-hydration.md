...
не существует. Тогда используй [`ssrShell`](./synapse-ctx.md#ssr--фоновые-провайдеры-без-серверных-данных-ssrshell):
модуль строит на сервере синхронный «пустой» стор из `initialState`, чтобы его `children` попали в HTML,
а на клиенте апгрейдится до реального async-стора.
