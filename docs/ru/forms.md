...
**TL;DR.** Форма как хранилище synapse — это state формы (`values`/`errors`/`touched`/…), точечные
подписки на поля, валидация как middleware, persist и синхронизация между вкладками **из коробки, без
form-библиотек**. Это **рецепт** (copy-paste код, не публичное form-API): берёте нужный уровень
лесенки и правите под себя.
...
- Форме нужны **SSR + гидрация**, **persist черновика**, **синхронизация между вкладками** или
  **централизованная валидация** — то, что form-библиотеки прикручивают сбоку, а хранилище даёт по
  архитектуре.
- Вы уже держите состояние на synapse и не хотите тянуть отдельную form-библиотеку ради одной формы.
- Нужны **точечные ре-рендеры по полю** без ручной мемоизации — `useStorageSubscribe` изолирует их
  сам.

## Когда НЕ нужно

- **Простая форма без SSR/persist/межвкладочности и без синхронизации со стором** — нативная
  `react-hook-form` / `Formik` закроет её быстрее; рецепт ниже избыточен.
- **Нужен готовый field-registration** (`register('email')` под нативные inputs), богатая
  async-валидация и array-fields из коробки — здесь этого нет намеренно (см. «Честные границы»).
