...
**TL;DR:** `version` + `migrate` в конфиге персистентного хранилища переводят сохранённые данные старой схемы к текущей при `initialize()` — без ручной проверки версий и без потери пользовательских данных.

## Зачем

Когда форма `initialState` меняется между релизами, в персистентном хранилище (`LocalStorage` / `IndexedDBStorage`) остаются данные **старой схемы**. Опции конфига `version` и `migrate` позволяют преобразовать их к текущей схеме один раз при инициализации.

Для `MemoryStorage` опции игнорируются (нечего персистить). Без `version` поведение не меняется — миграция выключена.

## Когда использовать

- Персистентный стор (`LocalStorage`/`IndexedDB`), у которого **менялась форма** `initialState` между релизами.
- Нужно сохранить пользовательские данные при апгрейде схемы (переименование/переструктурирование полей).

## Когда НЕ нужно

- `MemoryStorage` — данные не персистятся, миграция не имеет смысла (опции игнорируются).
- Схема стабильна и не меняется — `version`/`migrate` не нужны.
- Данные засеваются серверным снапшотом через [`hydrate`](./ssr-hydration.md) — снапшот считается уже актуальным, миграция на нём не запускается.
